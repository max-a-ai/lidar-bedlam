...
#   setsid nohup slurm/wandb_mirror.sh > outputs/wandb_mirror.log 2>&1 < /dev/null &
set -uo pipefail
cd "$(dirname "$0")/.."
export UV_PROJECT_ENVIRONMENT=${UV_PROJECT_ENVIRONMENT:-$HOME/venvs/lidar-bedlam}
INTERVAL=${INTERVAL:-120}
while true; do
  uv run python scripts/wandb_mirror.py --root outputs --once 2>&1 | grep -v "^wandb: " | grep -v "^$"
