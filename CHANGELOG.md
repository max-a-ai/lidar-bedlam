...
- Fixes found while running at scale: the BEDLAM per-frame mask lookup ignored the group (failed with several groups), SLOPER4D seq009 stores 4 values per 2D joint (parser now takes the first 3), a bash variable named `GROUPS` is reserved. Background: synthetic generation of the ten finished groups (117,657 frames), Waymo shards, SLOPER4D test shards, token precompute. Helma probe: compute nodes reach PyPI, wandb API not reachable (offline mode + sync), `$WORK` not visible from the probe job.
