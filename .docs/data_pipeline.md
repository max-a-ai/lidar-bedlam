...
| Legacy SMPL pkl conversion | `lidar_bedlam/body/convert_smpl.py` | run once, output under `resources/data/generated/body_models` |
...
`notebooks/waymo_pseudo_gt.ipynb` (generated by
`lidar_bedlam/scripts/build_pseudo_gt_notebook.py`) is the viewer for the
Waymo pseudo ground truth alone: three rotatable 3D panels per page, each
one accepted record with its fitted SMPL mesh, its LiDAR returns and its
labelled 3D keypoints, and buttons that page through all 4,586 accepted
records. The order is a fixed permutation, so the first page is always
the same three samples.

Its second section puts our fit and the pedestrian-generation project's fit on
the same record. Their pseudo ground truth lives on the NAS under
`methods/ped_gen/waymo/processed/` (4,812 tracks in 407 GiB of JSON, each frame
carrying a full 6890-vertex mesh, plus a 962-track, 81.5 GiB eval split; 816 GiB
for the whole folder). The two label sets share no identifier: we key a record
by the Waymo frame timestamp and the camera object id, they key a track by their
own scene index and instance index, whose ids are base64 Waymo *laser* ids. They
do share the Waymo world frame, so `lidar_bedlam/scripts/match_ped_gen_records.py`
pairs each of our records with the instance frame nearest to it in world
coordinates: 1,661 of our 4,586 accepted fits match, at a median distance of
0.0 cm, over 489 of their tracks.

`lidar_bedlam/scripts/cache_ped_gen_smpl.py --match` reads those tracks once and
caches their meshes in the camera frame of our record. Their track frame is the
ego frame of the track's first frame with the axes permuted
(`ego x, y, z = their z, x, y`), verified per track against the scene's ego poses
(residual 0.00 cm on clean tracks). Their meshes are exactly
`SMPL(body_pose, betas)` rotated by `ped_orientation` and placed at the mesh
centroid, so the cache stores parameters, centroid, joints and 64 reference
vertices instead of meshes, and the notebook rebuilds them to 0.00 mm. Without
`--match` the script caches a random sample of tracks instead
(`waymo_smpl_cache.npz`).

Of the 1,622 cached frames, 1,489 come from tracks whose ego-frame check is
clean (under 5 cm). Those same 1,489 records also carry a LiDAR-HMR fit, matched
by `lidar_bedlam/scripts/match_lidar_hmr_records.py`: their records name the
Waymo segment, frame timestamp and laser object id and hold the mesh in the
vehicle frame, so a record is paired with the fit in the same frame whose mesh
centre is nearest on the ground plane. All 4,586 of our records match at a median
of 7.9 cm, because their `train.pkl` covers every frame we use. Their mesh is
reproduced exactly by our SMPL model from the stored parameters with the
63-vector body pose padded to 69, so only parameters are cached. Both matchers
share `lidar_bedlam/data/waymo_index.py`, which reads the extraction pickles for
the frame timestamp, the box centre in the vehicle and world frames and the
transforms into the record's camera.

The notebook's second section is therefore one record per page, shown three
times, with the camera crop below. The pedestrian-generation mesh arrives about
29 cm too low, because a track is stored relative to the ego vehicle at the
origin, so its height is measured from the vehicle origin rather than the road;
it is lifted by the vertical part of the translation that best fits its joints to
the Waymo keypoints and is also drawn as delivered. `VERTICAL_ONLY = False`
switches to a full three-axis fit. Medians over 150 of the 1,489 records, in
centimetres:

| | LiDAR point-to-mesh | keypoint error |
|---|---|---|
| ours | 2.8 | 2.0 |
| ped_gen, lifted | 5.7 | 13.7 |
| LiDAR-HMR | 2.9 | 3.1 |

Its third section shows the pseudo ground truth LiDAR-HMR published and trains
on (`resources/data/external/lidar_hmr_waymov2/`, `train.pkl` 2.5 GB and
`test.pkl` 571 MB / 1,873 records, the one the notebook loads). A record holds
the fitted mesh, its SMPL parameters, 14 keypoints, the person's LiDAR points
and a `location_dict` with the Waymo segment, frame timestamp and laser object
id. Everything shares the Waymo vehicle frame, so the section needs no matching.
Medians over 200 of their test records: point-to-mesh 3.1 cm, keypoint error
2.6 cm over their 12 limb joints, 284 returns at 14.7 m. Their id and timestamp
would also support a three-way comparison on one person, which
`match_ped_gen_records.py` does not cover yet.

`notebooks/capabilities.ipynb` (generated by `lidar_bedlam/scripts/build_debug_notebook.py`,
executed copy `notebooks/capabilities.executed.ipynb` is git-ignored) shows five
...
`lidar_bedlam/scripts/export_rigs.py` writes every rig to `.docs/figures`
(`.docs/figures/rigs/<slug>.{png,html,glb,txt}`) and the note
