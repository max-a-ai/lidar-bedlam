...
% Venue names for refs.bib: switch to bib/bib-long.def for full names.
\input{bib/bib-short.def}
...
\todo{Write abstract.}
\begin{itemize}
  \item The problem (general wording)
  \item Proposed Solution
  \item Main Contribution(s)
  \item Experimental Set-up
  \item "Headline Result"
  \item Open-source link
\end{itemize}
...
\begin{itemize}
  \item \textbf{A LiDAR-enhanced synthetic dataset for SMPL pose and
    ...
    camera-LiDAR dataset with SMPL annotation.
  \item \textbf{A selective-attention fusion model.} Per-query gates route
    ...
    LiDAR, emphasizing strengths of the two modalities.
  \item \textbf{Extensive experiments and evaluation on two challenging
    in-the-wild datasets.} On Waymo and SLOPER4D we show (hopefully) taht our approach
    ...
    data mixture, and synthesis choices.
\end{itemize}
...
\begin{itemize}
  \item Add pioneerrs
  \item Add image-approaches
  \item LiDAR and multi-modal approaches
  \item limitations, funnel to the main standing problem - no real depth from image only but no large scale dataset
  \item Examples citations: 4d humans, tokenhmr, camerahmr, sam3dbody, smplify. mention vitpose and dino as image encoder.
  \item Mention depth ambiguity, and problems like thinking that children are basically people very far away 
\end{itemize}

\subsection{Human Motion Datasets}
% TODO: LiDARCap / LiDARHuman26M~\cite{li2022lidarcap},
% LiDAR-HMR~\cite{fan2025lidarhmr}, SLOPER4D baselines~\cite{dai2023sloper4d};
% sparse points give metric position but weak pose/shape detail.
\begin{itemize}
  \item Image datasets with SMPL labels (Human3.6M, 3DPW, AGORA, BEDLAM);
    what trained the current image estimators.
  \item LiDAR and camera datasets with SMPL labels: LiDARHuman26M,
    SLOPER4D, Human-M3, HuCenLife, CIMI4D; small, few rigs, mostly
    single subject or single scene.
  \item AV and VRU datasets with humans: Waymo~\cite{sun2020waymo,zheng2022waymokeypoints}
    (3D keypoints, no mesh), nuScenes~\cite{caesar2020nuscenes} and
    KITTI~\cite{geiger2012kitti} (boxes only),
    BikeActions~\cite{buettner_icpr2026_bikeactions} (camera and LiDAR recorded
    from a bicycle in urban scenes, 3D skeletons and actions); large or
    close-range but without SMPL labels.
  \item No real dataset provides dense depth; only synthetic BEDLAM does,
    with SMPL-X, intrinsics and clothing.
  \item No synthetic toolbox exists to create camera--LiDAR SMPL samples
    for a given sensor rig.
  \item Multi-modal training needs more data than single-modality training
    (to be proven: data-scaling ablation, Sec.~\ref{sec:ablations});
    the available multi-modal data is not sufficient, a large data
    backbone is needed.
  \item Domain gap: for LiDAR the gap is geometric (placement, resolution,
    occlusion, noise) and can be closed by simulation choices, unlike the
    appearance gap of images.
\end{itemize}
...
\paragraph{Metrics.}
MPJPE, PA-MPJPE and PVE are root-relative: the predicted and the
labelled joints (vertices) are centred on their pelvis before the mean
Euclidean distance is taken, PA-MPJPE after an additional similarity
alignment. They measure pose and shape only and cannot see where a method
puts the person. Because image-only regressors are depth-ambiguous, we
also report the \emph{absolute} joint error (mean joint distance without
any centring, pose and placement together) and the \emph{placement
error}, the distance between the predicted and the labelled pelvis
(SLOPER4D) or hip centre (Waymo, over the records with both hips
labelled). Detection quality is the mean AP of the predicted 3D box over
IoU thresholds $0.25$, $0.5$ and $0.7$; methods without a box head get
the oriented extent of their mesh, scaled to Waymo's padded box
convention. Waymo has no meshes, so PVE is reported on SLOPER4D only, and
its 15 keypoints are compared to the 13 COCO joints regressed from the
predicted mesh. Numbers of methods we cannot run on our records
(LiDARCap on SLOPER4D) are quoted from their papers and marked as such.
\todo{Describe datasets and splits.}

\subsection{Training}
\begin{itemize}
  \item One node with four H100s, batch 2048 (throughput saturates at
    1024 and memory peaks at 14\,GB per GPU in short tests from 256 to
    2048), AdamW with learning rate $3\cdot10^{-4}$, 500 warm-up steps,
    cosine decay, bf16, frozen ViT-H with precomputed tokens.
  \item Main-table runs use 10{,}200 steps for every mixture, i.e.\ equal
    compute; ablations share one mixture and use one third of it.
  \item Training reads \emph{shards}: files of 512 records, each a
    $256\times256$ person crop with mask and crop intrinsics, the SMPL or
    keypoint labels, the 3D box and the LiDAR returns of every scan
    variant, memory-mapped row by row; no raw dataset is touched during
    training.
  \item The frozen ViT-H features of every crop are computed once and
    stored next to its shard (fp16), so training skips the backbone: this
    halves the per-sample step time on one GPU (3.1 of 6.2\,ms) and removes
    over 90\,\% of the forward FLOPs.
  \item Discarded and rerun, not reported: a first mixed run whose Waymo
    keypoints were mapped to the wrong joints, and ablations on a
    preliminary 80/10/10 mixture.
  \item All trainings have been run at least 2 times with different seeds, ensuring that the results are stable and reproducible.
\end{itemize}

\subsection{Baselines}
% TODO: CameraHMR~\cite{patel2025camerahmr}, TokenHMR~\cite{dwivedi2024tokenhmr},
% LiDAR-HMR~\cite{fan2025lidarhmr}, SAM 3D Body~\cite{sam3dbody2025},
% LIF-Net~\cite{buettner_iv2026_lifnet}; how each is evaluated for translation / boxes.
\todo{Describe baselines.}

\subsection{Main results}
\subsection{Quantitative Results}
\begin{itemize}
  \item We show that a mix of real significantly improves performance over both synthetic-only and real-only training, especially in translation and box metrics.
\end{itemize}
...
\begin{table*}[t]
  ...
  \begin{tabular}{@{}llcrrrrrr@{}}
    ...
    LiDAR-HMR~\cite{fan2025lidarhmr}    & LiDAR & real      & -- & -- & -- & -- & -- & -- \\
    LIF-Net~\cite{buettner_iv2026_lifnet}               & RGB+LiDAR & real  & -- & -- & -- & -- & -- & -- \\
    \midrule
...
\subsection{Comparison: train on synthetic, real, mixed}
% Decided plan: mix the data that is used for trianing.
% try training on the mixed data, synthetic only, real only.
