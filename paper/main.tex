...
\begin{itemize}
  \item \textbf{A LiDAR-enhanced synthetic dataset for SMPL pose and
    ...
    a large-scale synthetic image dataset and keep its SMPL labels; virtual
    distance, occlusion and sensor augmentations, together with a
    camera-frame 3D box per person, make every sample an image--LiDAR
    training pair.
  ...
  \item \textbf{Evidence that synthetic data improves in-the-wild
    estimation, including 3D placement.} On Waymo and SLOPER4D we report
    pose, shape and detector-style placement metrics, and ablate one
    ingredient at a time: fusion, data mixture, and synthesis choices.
\end{itemize}
