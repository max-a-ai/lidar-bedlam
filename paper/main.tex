...
% TODO: contributions list:
% (i)   a synthetic LiDAR-camera SMPL dataset generated from BEDLAM depth,
%       with Ouster-style beam presets, rolling shutter and randomised
%       sensor placement in a ball around the camera;
% (ii)  a selective-attention fusion model that routes 3D cues
%       (translation, box, depth-dependent joints) to LiDAR tokens and
%       semantic cues (pose, shape, identity) to image tokens;
% (iii) evidence that synthetic data improves real-world SMPL estimation,
%       including camera-frame 3D placement, on SLOPER4D, LiDARHuman26M
%       and Waymo.
